"""Experiment 5 — Figure v2 (re-render only).

Loads results/exp5_results.json and re-renders Figure 3 with two panels:
...
def _generate_figure_v2(results: List[Dict[str, Any]]) -> Path:
    ...
    plt.figtext(
        ...
        "Site 2 (Long Beach VA, n_test=16) is the worst site on calibration in 12/12 runs at every ε.",
        ha="center",
        fontsize=8,
        wrap=True,
    )
    ...
    fig_png = Path("figures") / "exp5_dp_v2.png"
    fig_pdf = Path("figures") / "exp5_dp_v2.pdf"
...
def _print_reframed_interpretation(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    print(
        f"  Site 2 (Long Beach VA, n_test=16) is the worst site on calibration intercept "
...
def _write_paper_text_tex(stats: Dict[str, Any]) -> Path:
    ...
    text = (
        ...
        r"Site~2 (Long Beach VA, $n_{\text{test}}{=}16$) is the worst site on calibration "
...
def _write_paper_figure_tex() -> Path:
    text = (
        r"\begin{figure}[t]" "\n"
        r"\centering" "\n"
        r"\includegraphics[width=\linewidth]{figures/exp5_dp_v2.pdf}" "\n"
        ...
        r"Bands are mean $\pm 1.96 \times$ SE across seeds. Site~2 (Long Beach VA, "
