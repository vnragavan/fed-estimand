...
def main() -> None:
    ...
    boot = pd.read_csv("results/exp1_v2_ci_summary.csv")
    ...
    fig_png = Path("figures") / "exp1_pareto_v3.png"
    fig_pdf = Path("figures") / "exp1_pareto_v3.pdf"
    fig.savefig(fig_png, dpi=300, bbox_inches="tight")
    fig.savefig(fig_pdf, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print("Across-seed SE-based CIs (the alpha effect):")
    for r in rows:
        print(
    ...
    print()
    print("Pairwise comparison (alpha=0 vs alpha=1, mean ± 1.96*SE):")
    print(f"Patient loss CIs overlap?    {'YES' if pl_overlap else 'NO'}")
    print(f"Worst-site loss CIs overlap? {'YES' if ws_overlap else 'NO'}")

    print()
    print("Patient-level bootstrap CI widths (uncertainty about population loss for fixed model):")
    for r in rows:
        print(
