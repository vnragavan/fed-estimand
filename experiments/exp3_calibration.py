...
def main() -> None:
    ...
    fig_png = Path("figures") / "exp3_calibration.png"
    fig_pdf = Path("figures") / "exp3_calibration.pdf"
    ...
    if pooled_auc < 0.70:
        ...
        print(
            ...
            f"{min_intercept:+.2f} to {max_intercept:+.2f}). The smallest site (Site 2, n_test="
            ...
            f"but Site 3 (n_test={next(s['n_test'] for s in site_results if s['site'] == 3)})\n"
    ...
    print("=" * 66)
    try:
        with open("results/exp1_v2_results.json", "r", encoding="utf-8") as f:
        ...
        site_label_full = {0: "0 (Cleveland)   ", 1: "1 (Hungary)     ", 2: "2 (Long Beach)  ", 3: "3 (Switzerland) "}
