"""Experiment 5 — DP privacy–utility training sweep on Fed-Heart-Disease.

Trains federated models at multiple ε (plus no-DP baseline), saves scalar metrics,
and writes a diagnostic two-panel figure to ``figures/exp5_original.{png,pdf}``.
For the paper-style pooled-vs-worst-site plot + LaTeX snippets, run
``python -m experiments.exp5_figure`` after this completes.
...
def _generate_figure(results: List[Dict[str, Any]]) -> None:
    ...
    fig_png = Path("figures") / "exp5_original.png"
    fig_pdf = Path("figures") / "exp5_original.pdf"
